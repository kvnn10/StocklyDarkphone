...
const asQuantity = (value: bigint | number) => {
  ...
  if (!Number.isSafeInteger(quantity) || quantity < 0) throw new Error("Stock quantity must be a non-negative integer");
  return quantity;
};
...
export async function getStockAllocationsByProduct(productId: string) { return prisma.stockAllocation.findMany({ where: { productId }, orderBy: { createdAt: "desc" } }); }
export async function getStockAllocationsByWarehouse(warehouseId: string) { return prisma.stockAllocation.findMany({ where: { warehouseId }, orderBy: { createdAt: "desc" } }); }
export async function getStockAllocationById(id: string) { return prisma.stockAllocation.findUnique({ where: { id } }); }
...
export async function upsertStockAllocation(data: CreateStockAllocationInput, userId: string) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const product = await tx.product.findFirst({ where: { id: data.productId, userId }, select: { id: true, name: true, sku: true } });
    if (!product) throw new Error("Product not found or access denied");
    const warehouse = await tx.warehouse.findFirst({ where: { id: data.warehouseId, userId }, select: { id: true, name: true } });
  ...
  await writeAuditLog({ userId, action: "STOCK_ALLOCATION_UPDATED", entityType: "StockAllocation", entityId: result.allocation.id, details: { productId: data.productId, productName: result.product.name, sku: result.product.sku, warehouseId: data.warehouseId, warehouseName: result.warehouse.name, previousQuantity: result.previousQuantity, quantity, delta: quantity - result.previousQuantity } });
...
export async function updateStockAllocation(id: string, data: UpdateStockAllocationInput) {
  ...
  await writeAuditLog({ userId: result.allocation.userId, action: "STOCK_ALLOCATION_UPDATED", entityType: "StockAllocation", entityId: id, details: { productId: result.existing.productId, warehouseId: result.existing.warehouseId, previousQuantity: Number(result.existing.quantity), quantity: Number(result.allocation.quantity) } });
  return result.allocation;
}
...
export async function deleteStockAllocation(id: string) {
  ...
  await writeAuditLog({ userId: result.existing.userId, action: "STOCK_ALLOCATION_DELETED", entityType: "StockAllocation", entityId: id, details: { productId: result.existing.productId, warehouseId: result.existing.warehouseId, quantity: Number(result.existing.quantity) } });
  return result.allocation;
}

export async function getStockTransfers(userId: string) { return prisma.stockTransfer.findMany({ where: { userId }, orderBy: { createdAt: "desc" } }); }
export async function getStockTransferById(id: string, userId: string) { return prisma.stockTransfer.findFirst({ where: { id, userId } }); }
...
export async function createStockTransfer(data: CreateStockTransferInput, userId: string) {
  ...
  await writeAuditLog({ userId, action: "STOCK_TRANSFER_CREATED", entityType: "StockTransfer", entityId: transfer.id, details: { productId: data.productId, fromWarehouseId: data.fromWarehouseId, toWarehouseId: data.toWarehouseId, quantity } });
  return transfer;
}

export async function completeStockTransfer(id: string, userId: string) {
  const transfer = await prisma.$transaction(async (tx) => {
    const current = await tx.stockTransfer.findFirst({ where: { id, userId } });
    if (!current) throw new Error("Transfer not found or access denied");
    if (current.status !== "pending") throw new Error("Transfer is not pending");
    const quantity = asQuantity(current.quantity);
    const source = await tx.stockAllocation.findUnique({ where: { productId_warehouseId: { productId: current.productId, warehouseId: current.fromWarehouseId } } });
...
export async function cancelStockTransfer(id: string, userId: string) {
  ...
  const cancelled = await prisma.stockTransfer.update({ where: { id }, data: { status: "cancelled" } });
  await writeAuditLog({ userId, action: "STOCK_TRANSFER_CANCELLED", entityType: "StockTransfer", entityId: id, details: { productId: transfer.productId, fromWarehouseId: transfer.fromWarehouseId, toWarehouseId: transfer.toWarehouseId, quantity: Number(transfer.quantity) } });
  return cancelled;
}
...
export async function getWarehouseStockSummary(userId: string) {
  ...
  return warehouses.map((wh) => { const warehouseAllocations = allocations.filter((a) => a.warehouseId === wh.id); const totalProducts = warehouseAllocations.length; const totalQuantity = warehouseAllocations.reduce((sum, a) => sum + Number(a.quantity), 0); const totalReserved = warehouseAllocations.reduce((sum, a) => sum + Number(a.reservedQuantity), 0); const totalValue = warehouseAllocations.reduce((sum, a) => sum + Number(a.quantity) * (priceMap.get(a.productId) || 0), 0); return { warehouseId: wh.id, warehouseName: wh.name, warehouseType: wh.type ?? null, totalProducts, totalQuantity, totalReserved, availableQuantity: totalQuantity - totalReserved, totalValue }; });
}
