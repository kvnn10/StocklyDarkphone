...
/**
 * Create a new order and reserve its stock as one logical operation.
 * If reservation fails after the order is persisted, the order is deleted
 * immediately; reservePendingOrderLines also rolls back reservations already
 * created by the same call. This prevents orphan orders without stock holds.
 */
export async function createOrder(
  ...
  for (const item of data.items) {
    ...
    if (needsPick) {
      ...
      availableStock = getOrderLineCatalogAvailable(
        productQty,
        productReserved,
        allocationRows.map((row) => ({ reservedQuantity: Number(row.reservedQuantity ?? 0) })),
      );
    } else {
  ...
  try {
    await reservePendingOrderLines(
      productsToReserve.map((p) => ({ productId: p.id, quantity: p.qty, warehouseId: p.warehouseId })),
    );
  } catch (reservationError) {
    try {
      await prisma.order.delete({ where: { id: order.id } });
    } catch (cleanupError) {
      logger.error("Failed to remove order after reservation failure", {
        orderId: order.id,
  ...
  await Promise.all([
    ...
    console.error("Failed to invalidate product cache after order creation:", error);
  });
...
export async function getOrdersByUser(userId: string) {
  return prisma.order.findMany({
    where: { userId, OR: [{ clientId: null }, { clientId: userId }] },
    include: { items: { include: { product: { select: { id: true, name: true, sku: true, price: true } } } } },
  ...
  return prisma.order.findFirst({
    ...
    include: {
      items: {
        include: {
          product: {
            select: { id: true, name: true, sku: true, price: true, userId: true, categoryId: true, supplierId: true, imageUrl: true },
          },
        },
      },
    },
...
export async function getOrdersByClientId(clientId: string) {
  return prisma.order.findMany({
    where: { clientId },
    include: { items: { include: { product: { select: { id: true, name: true, sku: true, price: true, userId: true } } } } },
    orderBy: { createdAt: "desc" },
  });
}
