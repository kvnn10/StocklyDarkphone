...
async function getAccessibleProduct(id: string, session: { id: string; role: string }) {
  let accessWhere = {};
  if (session.role === "supplier") {
    const supplier = await getSupplierByUserId(session.id);
    if (!supplier) return null;
    accessWhere = { supplierId: supplier.id };
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const product = await getAccessibleProduct(id, session);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (warehouseId) {
      const warehouse = await prisma.warehouse.findFirst({ where: { id: warehouseId, userId: product.userId }, select: { id: true } });
    ...
    const aggregateQuantity = await prisma.productVariant.aggregate({ where: { productId: product.id }, _sum: { quantity: true } });
    const legacyStock = await prisma.stockAllocation.aggregate({ where: { productId: product.id, userId: product.userId }, _sum: { quantity: true } });
