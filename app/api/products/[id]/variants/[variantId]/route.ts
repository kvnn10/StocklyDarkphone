...
async function getOwnedVariant(sessionId: string, productId: string, variantId: string) {
  return prisma.productVariant.findFirst({ where: { id: variantId, productId, userId: sessionId }, include: { product: { select: { id: true, name: true } }, stocks: true } });
}

export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; variantId: string }> }) {
  const auth = await authorizeRequest(request, "products", "update");
  ...
  try {
    const existing = await getOwnedVariant(session.id, id, variantId);
    ...
    const message = error instanceof Error ? error.message : "No se pudo actualizar la variante";
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; variantId: string }> }) {
  const auth = await authorizeRequest(request, "products", "delete");
  if (auth.response) return auth.response;
  const session = auth.session!;
  const { id, variantId } = await params;
  if (!validObjectId(id) || !validObjectId(variantId)) return NextResponse.json({ error: "Variante inválida" }, { status: 400 });
  const variant = await getOwnedVariant(session.id, id, variantId);
  ...
  const orderItems = await prisma.orderItem.count({ where: { variantId } });
  if (orderItems > 0) return NextResponse.json({ error: "La variante tiene historial de ventas; desactívala en lugar de eliminarla" }, { status: 409 });
