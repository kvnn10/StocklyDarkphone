...
const TOOLS = [
  {
    name: "create_product",
    description:
      "Create one product in Stockly. Category and supplier are resolved automatically by existing name or ID. Duplicate SKUs are not created.",
...
async function createProduct(args: Record<string, unknown>) {
  ...
  if (!ownerEmail) {
    throw new Error("MCP_OWNER_EMAIL is not configured in Vercel");
  }
  ...
  if (!owner || owner.role !== "admin") {
    throw new Error("MCP_OWNER_EMAIL must belong to an admin user");
  }
  ...
  if (category.error || supplier.error) {
    throw new Error(
      [category.error, supplier.error].filter(Boolean).join("; "),
    );
  }
  ...
  const candidate = {
    ...
    status:
      typeof quantity === "number"
        ? calculateProductStatus(quantity)
        : undefined,
...
export async function POST(request: NextRequest) {
  ...
  if (method === "initialize") {
    const requested = text(body.params?.protocolVersion);
    const protocolVersion = requested === "2025-11-25" ? requested : PROTOCOL_VERSION;
    return jsonRpc(id, {
      protocolVersion,
      capabilities: { tools: {} },
      serverInfo: SERVER_INFO,
      instructions:
        "Stockly product management. Use create_product to add inventory items.",
    });
  }
...
export async function GET() {
  return new NextResponse("Stockly MCP endpoint. Use POST with Streamable HTTP.", {
    status: 405,
    headers: { Allow: "POST" },
  });
}
