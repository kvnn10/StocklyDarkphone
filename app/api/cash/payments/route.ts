...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const invoice = await tx.invoice.findFirst({ where: { id: invoiceId, userId: session.id } });
      ...
      await tx.salePayment.create({ data: { orderId: invoice.orderId, orderNumber: invoice.invoiceNumber, userId: session.id, recordedBy: session.id, amount, paymentMethod, status: "paid", createdAt: now } });
      const updatedInvoice = await tx.invoice.updateMany({ where: { id: invoice.id, userId: session.id, amountPaid: paid }, data: { amountPaid: newPaid, amountDue: newDue, status: invoiceStatus, sentAt: invoice.sentAt ?? now, paidAt: paymentStatus === "paid" ? now : null, updatedAt: now, updatedBy: session.id } });
      if (updatedInvoice.count !== 1) throw Object.assign(new Error("La factura cambió mientras se registraba el pago; vuelve a intentarlo"), { status: 409 });
      const updatedOrder = await tx.order.updateMany({ where: { id: invoice.orderId, userId: session.id }, data: { paymentStatus, updatedAt: now, updatedBy: session.id } });
      if (updatedOrder.count !== 1) throw Object.assign(new Error("No se pudo actualizar el estado de pago de la venta"), { status: 409 });
      await tx.cashMovement.create({ data: { type: "income", source: "sale", amount, paymentMethod, orderId: invoice.orderId, orderNumber: invoice.invoiceNumber, userId: session.id, createdBy: session.id, description: `Pago factura ${invoice.invoiceNumber}`, createdAt: now } });
      return { amountPaid: newPaid, amountDue: newDue, paymentStatus, invoiceStatus };
    });
    await writeAuditLog({ userId: session.id, action: "INVOICE_PAYMENT_RECORDED", entityType: "Invoice", entityId: invoiceId, details: { amount, paymentMethod, newPaid: result.amountPaid, amountDue: result.amountDue, invoiceStatus: result.invoiceStatus }, userAgent: request.headers.get("user-agent"), ipAddress: request.headers.get("x-forwarded-for")?.split(",")[0] ?? request.headers.get("x-real-ip") });
    return NextResponse.json({ ok: true, amountPaid: result.amountPaid, amountDue: result.amountDue, paymentStatus: result.paymentStatus, invoiceStatus: result.invoiceStatus });
  } catch (error) {
