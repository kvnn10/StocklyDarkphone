...
const priorityRank: Record<"critical" | "reorder" | "slow" | "healthy", number> = {
  critical: 0,
  reorder: 1,
  slow: 2,
  healthy: 3,
...
type Recommendation = {
  ...
  priority: "critical" | "reorder" | "slow" | "healthy";
  reason: string;
  suggestedQty: number;
};
...
export async function GET(request: NextRequest) {
  ...
  const recommendations: Recommendation[] = products.map((p) => {
    ...
    let priority: Recommendation["priority"] = "healthy";
    let reason = "Stock saludable para la velocidad actual.";
    let suggestedQty = 0;

    if (stock === 0 && stats.recentUnits > 0) {
      priority = "critical";
      reason = "Agotado y tiene ventas recientes.";
      suggestedQty = Math.max(1, Math.ceil(weeklyVelocity * 3));
    } else if (daysCover !== null && daysCover < 14 && stats.recentUnits > 0) {
      priority = "reorder";
      reason = `Aproximadamente ${Math.max(1, Math.round(daysCover))} días de cobertura.`;
      suggestedQty = Math.max(1, Math.ceil(weeklyVelocity * 4 - stock));
    } else if (stats.units === 0 && stock > 0) {
      priority = "slow";
      reason = "No registra ventas en el historial disponible; revisar antes de comprar más.";
    }
    ...
    return {
      ...
      sku: p.sku,
      stock,
      sold30d: stats.recentUnits,
      totalSold: stats.units,
      daysCover: daysCover === null ? null : Math.round(daysCover),
      marginPct,
      inventoryValue: stock * n(p.purchasePrice),
      priority,
      reason,
      suggestedQty,
    };
  })
    .filter((p) => p.priority !== "healthy")
    .sort((a, b) => priorityRank[a.priority] - priorityRank[b.priority] || b.sold30d - a.sold30d);
  ...
  return NextResponse.json({
    summary: {
      ...
      inventoryCost,
      inventoryRetail,
      potentialMargin: inventoryRetail - inventoryCost,
      critical: recommendations.filter((r) => r.priority === "critical").length,
      reorder: recommendations.filter((r) => r.priority === "reorder").length,
      slow: recommendations.filter((r) => r.priority === "slow").length,
    },
