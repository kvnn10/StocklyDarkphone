...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await mongo.connect();
    try {
      const serviceRows = await mongo.db().collection("ServiceOrder").find({ userId: session.id, createdAt: { $gte: since } }).toArray();
      serviceOrders = serviceRows.length;
      const productIds = [...new Set(serviceRows.flatMap((row: any) => Array.isArray(row.parts) ? row.parts.map((part: any) => part?.productId).filter(Boolean) : []))];
      const validObjectIds = productIds.filter((id: string) => ObjectId.isValid(id)).map((id: string) => new ObjectId(id));
      const products = validObjectIds.length
        ? await mongo.db().collection("Product").find({ userId: session.id, _id: { $in: validObjectIds } }).project({ purchasePrice: 1 }).toArray()
        : [];
