...
const movementSchema = z.object({
  ...
  referenceId: z.string().trim().max(200).optional().nullable(),
  notes: z.string().trim().max(500).optional().nullable(),
}).superRefine((value, ctx) => {
  if (value.type === "entry" && value.quantity <= 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["quantity"], message: "Entry quantity must be greater than zero" });
  }
  if (value.type === "exit" && value.quantity <= 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["quantity"], message: "Exit quantity must be greater than zero" });
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const productIds = productId
      ? (await findAccessibleProduct({ id: session.id, role: session.role }, productId, "read"))
        ? [productId]
        : []
      : undefined;

    if (productId && !productIds?.length) {
    ...
    const where = {
      ...
      ...(type && ["entry", "exit", "adjustment", "transfer_in", "transfer_out"].includes(type) ? { type } : {}),
    };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const warehouse = await prisma.warehouse.findFirst({ where: { id: data.warehouseId, userId: session.id }, select: { id: true } });
    if (!warehouse) return NextResponse.json({ error: "Warehouse not found" }, { status: 404 });
    ...
    const movement = await prisma.$transaction(async (tx) => {
      ...
      const delta = BigInt(data.quantity);
      const effectiveDelta = data.type === "entry" ? delta : data.type === "exit" ? -delta : delta;
      const newStock = previousStock + effectiveDelta;

      if (newStock < reserved) {
        throw new Error(`Insufficient available stock. Available: ${previousStock - reserved}`);
      }

      const updatedAllocation = allocation
        ? await tx.stockAllocation.update({
            where: { id: allocation.id },
            data: { quantity: newStock, updatedAt: new Date() },
          })
      ...
      await tx.product.update({ where: { id: data.productId }, data: { quantity: totalStock, updatedAt: new Date() } });
      ...
      return tx.inventoryMovement.create({
        data: {
          ...
          previousStock,
          newStock: updatedAllocation.quantity,
    ...
    if (message.includes("Insufficient")) return NextResponse.json({ error: message }, { status: 409 });
