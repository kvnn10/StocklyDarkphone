...
async function getOrderStats(clientIds: string[]) {
  ...
  try {
    const rows = await client.db().collection("Order").aggregate([
      ...
      { $group: { _id: "$clientId", orderCount: { $sum: 1 }, totalSpent: { $sum: "$total" } } },
    ]).toArray();
    return new Map(rows.map((row: any) => [String(row._id), { orderCount: Number(row.orderCount ?? 0), totalSpent: Number(row.totalSpent ?? 0) }]));
  } finally { await client.close(); }
}

export async function GET(request: NextRequest) {
  try {
    const auth = await authorizeRequest(request, "clients", "read");
    if (auth.response) return auth.response;
    const clients = await withClientsCollection(async (users) => users.find({ role: "client" }, { projection: { password: 0 } }).sort({ createdAt: -1 }).toArray());
    ...
    return NextResponse.json(clients.map((client: any) => {
      ...
      return { ...client, id, _id: undefined, orderCount: stats.orderCount, totalSpent: stats.totalSpent };
    }));
  } catch (error) { logger.error("Error fetching clients:", error); return NextResponse.json({ error: "No se pudieron cargar los clientes" }, { status: 500 }); }
}

export async function POST(request: NextRequest) {
  try {
    const auth = await authorizeRequest(request, "clients", "create");
    if (auth.response) return auth.response;
...
export async function PUT(request: NextRequest) {
  try {
    const auth = await authorizeRequest(request, "clients", "update");
    if (auth.response) return auth.response;
    const session = auth.session!;
    const body = await request.json();
...
export async function DELETE(request: NextRequest) {
  try {
    const auth = await authorizeRequest(request, "clients", "delete");
    if (auth.response) return auth.response;
    const session = auth.session!;
