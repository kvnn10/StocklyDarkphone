...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validationResult.success) {
      logger.warn("Invalid stock transfer request", { errors: validationResult.error.errors });
    ...
    const [fromWarehouse, toWarehouse] = await Promise.all([
      prisma.warehouse.findFirst({ where: { id: data.fromWarehouseId, userId: session.id } }),
      prisma.warehouse.findFirst({ where: { id: data.toWarehouseId, userId: session.id } }),
    ]);
    ...
    try {
      ...
      await cancelStockTransfer(transfer.id, session.id).catch((cancelErr) => {
        logger.warn("Failed to cancel pending stock transfer after complete error", { transferId: transfer.id, cancelErr });
      });
      throw completeError;
    }
