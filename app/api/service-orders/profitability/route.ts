...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = orders.map(order => {
      ...
      return { id: order.id, orderNumber: order.orderNumber, customer: String(m.customerName ?? ""), device: [order.brand, order.model].filter(Boolean).join(" "), status: order.status, revenue, partsRevenue, partsCost, labor, discount, grossProfit, margin, createdAt: order.createdAt };
    });
  ...
  } catch (error) { console.error("GET /api/service-orders/profitability", error); return NextResponse.json({ error: "No se pudo calcular la rentabilidad" }, { status: 500 }); }
}
