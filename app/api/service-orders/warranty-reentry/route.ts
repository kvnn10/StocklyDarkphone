...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const meta = object(original.accessories);
    const warrantyMap = meta.partWarranties ?? {};
    ...
    const accessories = {
      ...
      warrantyOriginalOrderId: original.id,
      warrantyOriginalOrderNumber: original.orderNumber,
      warrantyPartId: warrantyPart?.id ?? null,
      warrantyPartName: warrantyPart?.productName ?? null,
    ...
    const created = await prisma.serviceOrder.create({
      data: {
        ...
        reportedIssue: issue,
        initialCondition: original.initialCondition,
        accessories,
        status: "received",
        laborAmount: 0,
        partsAmount: 0,
        ...
        amountDue: 0,
        warrantyDays,
        warrantyExpiresAt: warrantyUntil ? new Date(warrantyUntil) : null,
        statusHistory: [{ status: "received", at: now.toISOString(), by: session.id }],
        createdAt: now,
        updatedAt: now,
    ...
    console.error("POST /api/service-orders/warranty-reentry", error);
    return NextResponse.json({ error: "No se pudo crear el reingreso por garantía" }, { status: 500 });
  }
}
