...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const order = await collection.findOne({ _id: new ObjectId(orderId), userId: session.id });
    if (!order) return NextResponse.json({ error: "Orden no encontrada" }, { status: 404 });
    if (["delivered", "cancelled"].includes(String(order.status))) return NextResponse.json({ error: "La orden está cerrada" }, { status: 409 });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!ObjectId.isValid(orderId) || !photoId) return NextResponse.json({ error: "Datos inválidos" }, { status: 400 });
  const client = await db();
  try {
    const collection = client.db().collection("ServiceOrder");
    const order = await collection.findOne({ _id: new ObjectId(orderId), userId: session.id });
    if (!order) return NextResponse.json({ error: "Orden no encontrada" }, { status: 404 });
    if (["delivered", "cancelled"].includes(String(order.status))) return NextResponse.json({ error: "La orden está cerrada" }, { status: 409 });
    await collection.updateOne({ _id: order._id }, { $pull: { photos: { id: photoId } }, $set: { updatedAt: new Date(), updatedBy: session.id } });
