...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const expiredIds = quotes.filter(qt => normalizeExpiration(qt, now).status === "expired" && qt.status !== "expired").map(qt => qt._id).filter(Boolean);
    if (expiredIds.length) await collection.updateMany({ _id: { $in: expiredIds }, userId: session.id }, { $set: { status: "expired", updatedAt: now } });
    return NextResponse.json(quotes.map(qt => normalizeExpiration(qt, now)));
