...
export async function GET(request: NextRequest) {
  ...
  if (days === null) return NextResponse.json({ error: `days debe ser un entero entre 0 y ${MAX_DAYS}` }, { status: 400 });
  const now = new Date();
  ...
  const mapEntry = (row: any, type: "receivable" | "payable") => {
    ...
    return { id: row._id?.toHexString?.() ?? String(row._id), type, reference: row.orderNumber ?? row.reference ?? null, partyId: row.clientId?.toHexString?.() ?? row.supplierId?.toHexString?.() ?? null, partyName: row.clientName ?? row.supplierName ?? null, amountDue, dueDate, bucket: classify(dueDate, now, horizon), daysLate: dueDate && dueDate < now ? Math.max(1, Math.ceil((now.getTime() - dueDate.getTime()) / 86400000)) : 0 };
  };
  const entries = [...receivables.map((row: any) => mapEntry(row, "receivable")), ...payables.map((row: any) => mapEntry(row, "payable"))];
  ...
  return NextResponse.json(jsonSafe({ generatedAt: now, days, summary: { totalOpen: sum(entries), overdue: sum(overdue), dueToday: sum(today), dueSoon: sum(next), overdueCount: overdue.length, dueTodayCount: today.length, dueSoonCount: next.length, openCount: entries.length }, receivables: { totalOpen: sum(byType(entries, "receivable")), overdue: sum(byType(overdue, "receivable")), dueSoon: sum(byType(today, "receivable")) + sum(byType(next, "receivable")) }, payables: { totalOpen: sum(byType(entries, "payable")), overdue: sum(byType(overdue, "payable")), dueSoon: sum(byType(today, "payable")) + sum(byType(next, "payable")) }, alerts: { overdue, today, next, later } }));
}
