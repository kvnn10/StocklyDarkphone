...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(jsonSafe({ purchase, payable, payments, cash, inventory, reconciliation: { purchaseTotal: purchase.total, receivedSubtotal, payableAmount: Number(payable?.originalAmount ?? 0), paid, expectedDue, cashTotal, inventoryUnits, cashMatchesPayments: Math.abs(cashTotal - paid) <= 0.01, payableMatchesPayments: Math.abs(expectedDue - Number(payable?.amountDue ?? 0)) <= 0.01 } }));
}
