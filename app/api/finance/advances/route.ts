...
export async function GET(request: NextRequest) {
  const { session, response } = await authorizeRequest(request, "finance", "read");
...
export async function PATCH(request: NextRequest) {
  const { session, response } = await authorizeRequest(request, "finance", "apply_advance");
  if (response) return response;
  try {
    const body = await request.json();
    ...
    const targetUpdate = { $set: { amountPaid: Number(target.amountPaid ?? 0) + amount, amountDue: nextDue, status: nextDue <= 0.0001 ? "paid" : "partial", updatedAt: now } };
