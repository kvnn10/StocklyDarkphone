...
export default function SalesPage() {
  ...
  async function completeSale() {
    ...
    try {
      ...
      const paymentResponse = await fetch("/api/sales/pay", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ orderId: sale.id, paymentMethod }) });
      ...
      setCart([]); setClientId("none"); setDiscount(0); setShowPayment(false); setMessage(`Venta ${payment.orderNumber} pagada correctamente por ${money(payment.amount)}.`);
    ...
    <main className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_420px]">
        ...
        <section className="rounded-2xl border bg-card/80 p-4 shadow-sm backdrop-blur-xl sm:p-5">
          ...
          <div className="mt-5 space-y-3 border-t pt-4"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span>{money(subtotal)}</span></div><div className="flex items-center justify-between gap-3"><span className="text-sm text-muted-foreground">Descuento</span><Input type="number" min="0" max={subtotal} value={discount} onChange={(e) => setDiscount(Number(e.target.value) || 0)} className="h-9 w-32 text-right" /></div><div className="flex justify-between pt-2 text-xl font-bold"><span>Total</span><span className="text-emerald-600">{money(total)}</span></div>{message && <div className="rounded-lg border bg-muted/40 p-3 text-sm">{message}</div>}<Button onClick={() => setShowPayment(true)} disabled={cart.length === 0 || isSaving} className="mt-1 w-full bg-emerald-600 hover:bg-emerald-700">Continuar al pago</Button></div>
        </section>
      </div>
      {showPayment && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-2xl border bg-card p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-semibold">Confirmar pago</h2><p className="text-sm text-muted-foreground">Total a cobrar: {money(total)}</p></div><button type="button" onClick={() => setShowPayment(false)}><X /></button></div><div className="grid grid-cols-2 gap-3">{([['cash','Efectivo',Banknote],['card','Tarjeta',CreditCard],['transfer','Transferencia',ArrowRight],['other','Otro',ShoppingCart]] as const).map(([value,label,Icon]) => <button key={value} type="button" onClick={() => setPaymentMethod(value)} className={`rounded-xl border p-4 text-left transition ${paymentMethod === value ? 'border-emerald-500 bg-emerald-500/10 ring-2 ring-emerald-500/20' : 'hover:bg-muted'}`}><Icon className="mb-2 h-5 w-5 text-emerald-600" /><p className="font-medium">{label}</p></button>)}</div><Button disabled={isSaving} onClick={completeSale} className="mt-5 w-full bg-emerald-600 hover:bg-emerald-700">{isSaving ? "Registrando..." : `Cobrar ${money(total)}`}</Button></div></div>}
    </main>
  );
}
