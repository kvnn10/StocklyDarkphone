...
import { Plus, Trash2, ShoppingBag, CreditCard, Banknote, RefreshCw, PackagePlus, X, Eye, Copy, RotateCcw, Layers3 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { PageContentWrapper } from "@/components/shared";

type Product = { id: string; name: string; sku: string; purchasePrice?: number; price?: number; quantity?: number };
type Variant = { id: string; name: string; sku: string; purchasePrice?: number; price?: number; quantity?: number; attributes?: Record<string, unknown> | null };
...
const statusLabel: Record<string, string> = { received: "Recibida", partial: "Parcial", pending: "Pendiente", returned: "Devuelta", cancelled: "Anulada", draft: "Borrador" };
...
export default function PurchasesPage() {
  ...
  const [lines, setLines] = useState<Line[]>([{ productId: "", variantId: null, quantity: 1, unitCost: 0 }]);
  ...
  const [cancelTarget, setCancelTarget] = useState<Purchase | null>(null), [cancelReason, setCancelReason] = useState(""), [canceling, setCanceling] = useState(false);

  const loadVariants = async (productId: string) => {
    if (!productId || variantsByProduct[productId] || variantLoading[productId]) return variantsByProduct[productId] ?? [];
    setVariantLoading(cur => ({ ...cur, [productId]: true }));
    try {
      const r = await fetch(`/api/products/${productId}/variants`, { cache: "no-store" });
      const d = await r.json();
      const variants = r.ok && Array.isArray(d) ? d : [];
      setVariantsByProduct(cur => ({ ...cur, [productId]: variants }));
      return variants;
    } catch { setVariantsByProduct(cur => ({ ...cur, [productId]: [] })); return []; }
    finally { setVariantLoading(cur => ({ ...cur, [productId]: false })); }
  };
  ...
  const selectProduct = async (index: number, id: string) => {
    const p = products.find(x => x.id === id);
    updateLine(index, { productId: id, variantId: null, unitCost: Number(p?.purchasePrice ?? 0) });
    const variants = await loadVariants(id);
    if (variants.length === 0) updateLine(index, { productId: id, variantId: null, unitCost: Number(p?.purchasePrice ?? 0) });
  };
  const selectVariant = (index: number, variantId: string) => {
    const line = lines[index]; const variants = line?.productId ? (variantsByProduct[line.productId] ?? []) : []; const variant = variants.find(v => v.id === variantId);
    updateLine(index, { variantId: variantId || null, unitCost: Number(variant?.purchasePrice ?? products.find(p => p.id === line?.productId)?.purchasePrice ?? 0) });
  };
  ...
  const createProduct = async () => {
    ...
    setNewProductSaving(true); try { const r = await fetch("/api/products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newProduct.name.trim(), sku: newProduct.sku.trim(), purchasePrice, price, quantity: 0, status: "Stock Out", categoryId: newProduct.categoryId, supplierId }) }); const d = await r.json(); if (!r.ok) throw new Error(d.error || "No se pudo crear el producto"); setProducts(cur => [d, ...cur]); updateLine(newProductLineIndex, { productId: d.id, variantId: null, unitCost: purchasePrice }); setVariantsByProduct(cur => ({ ...cur, [d.id]: [] })); setNewProductOpen(false); setMessage(`Producto ${d.name} creado y agregado a la compra.`); } catch (e) { setMessage(e instanceof Error ? e.message : "No se pudo crear el producto"); } finally { setNewProductSaving(false); }
  };
  const save = async () => {
    setMessage(""); if (!supplierId || !warehouseId) return setMessage("Selecciona proveedor y almacén.");
    if (lines.some(l => !l.productId || l.quantity <= 0 || l.unitCost < 0)) return setMessage("Completa correctamente todos los productos.");
    const missingVariant = lines.find(l => (variantsByProduct[l.productId]?.length ?? 0) > 0 && !l.variantId);
  ...
  const duplicate = (p: Purchase) => { setSupplierId(suppliers.find(s => s.name === p.supplierName)?.id ?? supplierId); setWarehouseId(warehouses.find(w => w.name === p.warehouseName)?.id ?? warehouseId); setPaymentMode("paid"); setPaymentMethod("transfer"); setSupplierInvoice(""); setDiscount(String(p.discount || 0)); setTax(String(p.tax || 0)); setShipping(String(p.shipping || 0)); setNotes(`Duplicada de ${p.purchaseNumber}`); setLines(p.items.map(i => ({ productId: i.productId, variantId: i.variantId ?? null, quantity: i.orderedQuantity, unitCost: i.unitCost }))); p.items.forEach(i => { if (i.variantId) loadVariants(i.productId); }); setDetail(null); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return <PageContentWrapper><div className="space-y-6">
    ...
    <section className="rounded-xl border bg-card p-5 shadow-sm space-y-5">
      ...
      <div className="space-y-3">{lines.map((line, index) => { const variants = variantsByProduct[line.productId] ?? []; const loadingVariants = !!variantLoading[line.productId]; return <div key={index} className="grid gap-2 md:grid-cols-[1fr_150px_120px_160px_44px] items-end"><div className="text-sm space-y-1"><span className="block">Producto</span><div className="flex gap-2"><select className="h-10 min-w-0 flex-1 rounded-md border bg-background px-3" value={line.productId} onChange={e => selectProduct(index, e.target.value)}><option value="">Seleccionar producto...</option>{products.map(p => <option key={p.id} value={p.id}>{p.name} · {p.sku}</option>)}</select><Button type="button" variant="outline" size="icon" onClick={() => openNewProduct(index)} title="Crear producto nuevo"><PackagePlus className="h-4 w-4" /></Button></div></div><div className="text-sm space-y-1"><span className="flex items-center gap-1">{variants.length > 0 && <Layers3 className="h-3 w-3" />}Variante</span><select className="h-10 w-full rounded-md border bg-background px-3" value={line.variantId ?? ""} onChange={e => selectVariant(index, e.target.value)} disabled={!line.productId || loadingVariants}><option value="">{loadingVariants ? "Cargando..." : variants.length ? "Seleccionar..." : "Sin variante"}</option>{variants.map(v => <option key={v.id} value={v.id}>{v.name} · {v.sku}</option>)}</select></div><label className="text-sm space-y-1"><span>Cantidad</span><Input type="number" min="1" value={line.quantity} onChange={e => updateLine(index, { quantity: Math.max(1, Number(e.target.value) || 1) })} /></label><label className="text-sm space-y-1"><span>Costo unitario</span><Input type="number" min="0" value={line.unitCost} onChange={e => updateLine(index, { unitCost: Math.max(0, Number(e.target.value) || 0) })} /></label><Button variant="ghost" size="icon" onClick={() => setLines(cur => cur.length > 1 ? cur.filter((_, i) => i !== index) : cur)} title="Eliminar línea"><Trash2 className="h-4 w-4" /></Button></div>; })}<Button variant="outline" onClick={() => setLines(cur => [...cur, { productId: "", variantId: null, quantity: 1, unitCost: 0 }])}><Plus className="h-4 w-4" />Agregar producto</Button></div>
    ...
    {detail && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-4xl max-h-[90vh] overflow-auto rounded-xl border bg-card p-5 shadow-xl space-y-5"><div className="flex items-start justify-between"><div><h3 className="text-xl font-semibold">{detail.purchaseNumber}</h3><p className="text-sm text-muted-foreground">{detail.supplierName} · {detail.warehouseName || "—"} · {statusLabel[detail.status] || detail.status}</p></div><Button variant="ghost" size="icon" onClick={() => setDetail(null)}><X className="h-4 w-4" /></Button></div><div className="grid gap-3 md:grid-cols-4 text-sm"><div><span className="text-muted-foreground">Referencia</span><div>{detail.supplierInvoice || "—"}</div></div><div><span className="text-muted-foreground">Subtotal</span><div>{money(detail.subtotal)}</div></div><div><span className="text-muted-foreground">Descuento</span><div>{money(detail.discount)}</div></div><div><span className="text-muted-foreground">Total</span><div className="font-semibold">{money(detail.total)}</div></div></div><div className="overflow-x-auto rounded-lg border"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-3">Producto</th><th className="p-3">Pedido</th><th className="p-3">Recibido</th><th className="p-3">Devuelto</th><th className="p-3">Costo</th><th className="p-3">Cantidad</th></tr></thead><tbody>{detail.items.map(i => { const pending = i.orderedQuantity - i.receivedQuantity; const returnable = i.receivedQuantity - (i.returnedQuantity || 0); const max = detailAction === "return" ? returnable : pending; return <tr key={i.id} className="border-b last:border-0"><td className="p-3">{i.productName}{i.variantName ? <div className="text-xs font-medium">Variante: {i.variantName}</div> : null}<div className="text-xs text-muted-foreground">{i.sku}</div></td><td className="p-3">{i.orderedQuantity}</td><td className="p-3">{i.receivedQuantity}</td><td className="p-3">{i.returnedQuantity || 0}</td><td className="p-3">{money(i.unitCost)}</td><td className="p-3"><Input className="w-24" type="number" min="0" max={max} value={detailQty[i.id!] || ""} onChange={e => setDetailQty(q => ({ ...q, [i.id!]: Number(e.target.value) || 0 }))} placeholder={`Máx. ${max}`} /></td></tr>; })}</tbody></table></div><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => duplicate(detail)}><Copy className="h-4 w-4" />Duplicar compra</Button>{detail.status !== "cancelled" && <Button variant="outline" onClick={() => { setCancelTarget(detail); setCancelReason(""); }}><Trash2 className="h-4 w-4" />Anular compra</Button>}<Button onClick={() => runDetailAction("receive")} disabled={!!detailAction || detail.status === "cancelled"}>{detailAction === "receive" ? "Recibiendo..." : "Recibir cantidades"}</Button><Button variant="outline" onClick={() => runDetailAction("return")} disabled={!!detailAction || detail.status === "cancelled"}><RotateCcw className="h-4 w-4" />Devolver cantidades</Button><Button variant="outline" onClick={() => setDetail(null)}>Cerrar</Button></div></div></div>}

    {cancelTarget && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4"><div className="w-full max-w-md rounded-xl border bg-card p-5 shadow-xl space-y-4"><div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold">Anular compra {cancelTarget.purchaseNumber}</h3><p className="text-sm text-muted-foreground mt-1">La compra quedará registrada como anulada, se revertirá el inventario pendiente de devolución y se ajustarán Caja / cuentas por pagar.</p></div><Button variant="ghost" size="icon" onClick={() => !canceling && setCancelTarget(null)}><X className="h-4 w-4" /></Button></div><label className="text-sm space-y-1 block"><span>Observación obligatoria</span><textarea className="min-h-28 w-full rounded-md border bg-background px-3 py-2 text-sm" value={cancelReason} onChange={e => setCancelReason(e.target.value)} placeholder="Ej. Compra registrada por error, factura duplicada..." maxLength={500} disabled={canceling} /></label><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setCancelTarget(null)} disabled={canceling}>Cancelar</Button><Button variant="destructive" onClick={cancelPurchase} disabled={canceling || cancelReason.trim().length < 5}>{canceling ? "Anulando..." : "Confirmar anulación"}</Button></div></div></div>}
