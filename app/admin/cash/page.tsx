...
import { ArrowDownCircle, ArrowUpCircle, Banknote, Plus, Wallet, Ban } from "lucide-react";
...
export default function CashPage() {
  ...
  async function voidMovement(movement: Movement) {
    ...
    const confirmed = window.confirm(`¿Anular este movimiento de ${money(Number(movement.amount))}?\n\nEl movimiento quedará registrado como anulado y no afectará el saldo de Caja.`);
    if (!confirmed) return;

    const reason = window.prompt("Motivo de la anulación:", "Movimiento registrado por error")?.trim();
    ...
    try {
      const response = await fetch("/api/cash", {
        ...
        credentials: "include",
        body: JSON.stringify({ id: movement._id, reason: reason || "Movimiento anulado manualmente" }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "No se pudo anular");
      setMessage("Movimiento anulado correctamente.");
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Error al anular movimiento");
    } finally {
      setVoidingId(null);
    }
  }
  ...
  return (
    <main className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        ...
        <section className="rounded-2xl border bg-card/80 p-5 shadow-sm"><h2 className="mb-4 font-semibold">Movimientos recientes</h2><div className="space-y-2">{data.movements.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">Aún no hay movimientos.</p> : data.movements.map((movement) => {
          const voided = movement.status === "voided";
          return <div key={movement._id} className={`flex items-center justify-between gap-3 rounded-xl border p-3 ${voided ? "opacity-60" : ""}`}>
            <div className="flex min-w-0 items-center gap-3"><div className={`rounded-lg p-2 ${voided ? "bg-muted text-muted-foreground" : movement.type === "income" ? "bg-emerald-500/10 text-emerald-600" : "bg-red-500/10 text-red-600"}`}>{voided ? <Ban className="h-4 w-4" /> : movement.type === "income" ? <ArrowUpCircle className="h-4 w-4" /> : <ArrowDownCircle className="h-4 w-4" />}</div><div className="min-w-0"><p className={`truncate font-medium ${voided ? "line-through" : ""}`}>{movement.description || "Movimiento"}</p><p className="text-xs text-muted-foreground">{movement.paymentMethod} · {new Date(movement.createdAt).toLocaleString("es-CO")}{voided ? " · ANULADO" : ""}</p>{voided && movement.voidReason && <p className="truncate text-xs text-muted-foreground">Motivo: {movement.voidReason}</p>}</div></div>
