...
async function main() {
  ...
  for (const doc of docs as any[]) {
    ...
    const parts: LegacyPart[] = Array.isArray(doc.parts) ? doc.parts : [];
    const payments = Array.isArray(doc.payments) ? doc.payments : [];
    const warehouseEntries = parts
      .filter((p): p is LegacyPart & { id: unknown; warehouseName: unknown } => Boolean(p?.id) && Boolean(p?.warehouseName))
      .map((p) => [String(p.id), String(p.warehouseName)] as const);
    ...
    const accessories = {
      ...
      statusHistory: Array.isArray(doc.statusHistory)
        ? doc.statusHistory
        : [{ status: String(doc.status ?? "received"), at: new Date(doc.createdAt ?? Date.now()).toISOString(), by: userId }],
      photos: Array.isArray(doc.photos) ? doc.photos : [],
      partWarranties: Object.fromEntries(
        parts
          .filter((p) => p?.warrantyDays !== undefined && p?.id)
          .map((p) => [String(p.id), {
            warrantyDays: Number(p.warrantyDays ?? 0),
            warrantyUntil: p.warrantyUntil ? new Date(String(p.warrantyUntil)).toISOString() : null,
            warrantySource: String(p.warrantySource ?? "legacy"),
          }]),
      ),
      partWarehouses: Object.fromEntries(warehouseEntries),
    ...
    const created = await prisma.serviceOrder.create({
      data: {
        ...
        partsAmount: parts.reduce((sum, p) => sum + Number(p?.subtotal ?? 0), 0),
