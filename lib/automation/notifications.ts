...
export async function claimPendingNotifications(limit = 50) {
  const col = await collection();
  const docs = await col.find({ status: "pending", $or: [{ "delivery.whatsapp": { $exists: false } }, { "delivery.telegram": { $exists: false } }] }).sort({ createdAt: 1 }).limit(Math.min(Math.max(limit, 1), 100)).toArray();
  return docs;
}
