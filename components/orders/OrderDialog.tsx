...
export default function OrderDialog({
  ...
  const handleCreateClientInline = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result.client?.id) {
        throw new Error(result.error || "No se pudo crear el cliente");
      }

      const client: OrderClientOption = {
        id: String(result.client.id),
        name: String(result.client.name ?? name),
        email: String(result.client.email ?? email),
        phone: String(result.client.phone ?? newClientPhone.trim()),
        whatsapp: String(result.client.whatsapp ?? ""),
        status: true,
      };

      setOrderClients((prev) => [client, ...prev.filter((item) => item.id !== client.id)]);
      ...
      toast({
        ...
        description: `${client.name} quedó seleccionado para este pedido.`,
      });
    } catch (error) {
      logger.error("Inline client creation error:", error);
      toast({
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Select
                        ...
                        disabled={clientsLoading || creatingClient}
                      >
                        <SelectTrigger className={cn("h-11 w-full sm:flex-1", DIALOG_FORM_FIELD_VIOLET)}>
                          <SelectValue
                            placeholder={
                              clientsLoading
                                ? "Loading customers..."
                      ...
                      <Button
                        ...
                        className="h-11 shrink-0 rounded-xl border border-violet-400/30 bg-violet-500/20 text-white hover:bg-violet-500/30"
                      >
                    ...
                    {showNewClientForm && (
                      <div className="rounded-xl border border-violet-400/25 bg-white/5 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-white">Crear cliente</p>
                            <p className="text-xs text-white/50">Se guardará y quedará seleccionado automáticamente.</p>
                          </div>
                          <Button
                            type="button"
                            variant="ghost"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div className="space-y-1.5">
                            <Label className="text-white/80 text-xs">Nombre *</Label>
                            <input
                              value={newClientName}
                              onChange={(e) => setNewClientName(e.target.value)}
                              placeholder="Nombre del cliente"
                              autoComplete="name"
                              className={cn("h-10 w-full rounded-md px-3 text-sm text-white placeholder:text-white/40 outline-none", DIALOG_FORM_FIELD_VIOLET)}
                            />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-white/80 text-xs">Correo electrónico *</Label>
                            <input
                              type="email"
                          ...
                          <div className="space-y-1.5">
                            <Label className="text-white/80 text-xs">Teléfono</Label>
                            <input
                          ...
                          <div className="space-y-1.5">
                            <Label className="text-white/80 text-xs">Documento</Label>
                            <input
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            className="rounded-xl bg-violet-500/80 text-white hover:bg-violet-500"
                          >
                    ...
                    <p className="text-xs text-white/50">
                      Opcional. Selecciona un cliente para vincular este pedido con su historial.
                    </p>
                  </div>
                )}
