  ...
  return <>
    <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0"><span className="sr-only">Abrir menú</span><MoreVertical className="h-4 w-4 text-gray-600 dark:text-gray-300" /></Button></DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="border border-white/10 bg-gradient-to-br from-white/5 via-white/5 to-white/5 backdrop-blur-md shadow-lg">
        ...
        {onEdit != null && order.status !== "cancelled" && <DropdownMenuItem onClick={handleEditOrder} disabled={disableOrderActions} className="flex items-center gap-2"><Edit className="h-4 w-4" />Editar pedido</DropdownMenuItem>}
        ...
        {order.status !== "cancelled" && <DropdownMenuItem className="text-red-600 dark:text-red-400" onClick={handleCancelOrder} disabled={isDeleting || disableOrderActions}><Trash2 className="h-4 w-4" />{isDeleting ? "Cancelando..." : "Cancelar pedido"}</DropdownMenuItem>}
      </DropdownMenuContent>
    </DropdownMenu>
