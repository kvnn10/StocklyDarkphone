...
/**
 * Product Status Filter Dropdown — REQ-0037 glass badges.
 */
...
type ProductStatusOption = { value: ProductStatus; label: string };

const productStatuses: ProductStatusOption[] = [
  { value: "Available", label: "Disponible" },
  { value: "Stock Low", label: "Stock bajo" },
  { value: "Stock Out", label: "Agotado" },
];
...
export function StatusDropDown({ selectedStatuses, setSelectedStatuses }: StatusDropDownProps) {
  const [open, setOpen] = React.useState(false);

  function handleToggle(value: string) {
    setSelectedStatuses((prev) => prev.includes(value) ? prev.filter((status) => status !== value) : [...prev, value]);
  }
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className={cn("p-0 w-48 poppins", filterCommandPopoverClass("rose"), FILTER_COMMAND_INPUT_WRAPPER_CLASS)} side="bottom" align="center">
          <Command className="p-1 bg-transparent">
            <CommandInput placeholder="Filtrar por estado..." className="bg-transparent border-0 focus:ring-0 focus:outline-none text-gray-700 dark:text-white/80 placeholder:text-gray-500 dark:placeholder:text-white/40" />
            <CommandList>
              <CommandGroup>
                {productStatuses.map((status) => (
                  <FilterCommandCheckboxItem key={status.value} value={status.label} toggleValue={status.value} checked={selectedStatuses.includes(status.value)} onToggle={handleToggle}>
                    <ProductStockStatusBadge status={status.value} label={status.label} />
            ...
            <div className="flex flex-col gap-2 text-[23px]">
              ...
              <Button variant="ghost" className="text-[12px] mb-1 text-gray-700 dark:text-white/80 hover:text-gray-700 dark:hover:text-white hover:bg-rose-100 dark:hover:bg-white/10" onClick={clearFilters}>Limpiar filtros</Button>
            </div>
          </Command>
        </PopoverContent>
      </Popover>
