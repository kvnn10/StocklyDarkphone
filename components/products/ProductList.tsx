...
import { useProducts, useCategories, useSuppliers, useOrders, useDashboard, useSupplierPortalDashboard, useProductVariants } from "@/hooks/queries";
...
const ProductList = React.memo(function ProductList({ initialProducts, initialStats, initialSupplierPortal }: ProductListProps = {}) {
  ...
  return <div className="flex flex-col poppins">
    ...
    {isAdminProductsPage && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2 pb-6 items-stretch">
      ...
      <StatisticsCard title="Total Value" value={formatCurrency(dashboard?.totalInventoryValue ?? 0)} description="Total inventory value" icon={DollarSign} variant="violet" valueLoading={dashboardCardsLoading} badgeValuesLoading={dashboardCardsLoading} badges={[{ label: "Orders", value: formatCurrency(dashboard?.orderAnalytics?.totalRevenueExcludingCancelled ?? dashboard?.revenue?.fromOrders ?? 0) }, { label: "Invoices", value: formatCurrency(dashboard?.revenue?.fromInvoices ?? 0) }, { label: "Due", value: formatCurrency(dashboard?.invoiceAnalytics?.outstandingAmount ?? 0) }, { label: "Cancelled", value: formatCurrency(dashboard?.orderAnalytics?.cancelledOrderAmount ?? 0) }]} />
      <StatisticsCard title="Total Suppliers" value={dashboard?.counts.suppliers ?? 0} description="Suppliers" icon={Truck} variant="emerald" valueLoading={dashboardCardsLoading} badgeValuesLoading={dashboardCardsLoading} badges={[{ label: "Active", value: dashboard?.supplierStatusBreakdown?.active ?? 0 }, { label: "Inactive", value: dashboard?.supplierStatusBreakdown?.inactive ?? 0 }]} />
    ...
    {isUserProductsPage && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 items-stretch pb-6">
      ...
      <StatisticsCard title="Total Value" value={formatCurrency(productsPageStats?.totalInventoryValue ?? 0)} description="Total inventory value" icon={DollarSign} variant="violet" valueLoading={dashboardCardsLoading} badgeValuesLoading={dashboardCardsLoading} badges={[{ label: "Orders", value: formatCurrency(productsPageStats?.orderAnalytics?.totalRevenueExcludingCancelled ?? productsPageStats?.revenue?.fromOrders ?? 0) }, { label: "Invoices", value: formatCurrency(productsPageStats?.revenue?.fromInvoices ?? 0) }, { label: "Due", value: formatCurrency(productsPageStats?.invoiceAnalytics?.outstandingAmount ?? 0) }, { label: "Cancelled", value: formatCurrency(productsPageStats?.orderAnalytics?.cancelledOrderAmount ?? 0) }]} />
      <StatisticsCard title="Total Suppliers" value={productsPageStats?.counts.suppliers ?? 0} description="Suppliers" icon={Truck} variant="emerald" valueLoading={dashboardCardsLoading} badgeValuesLoading={dashboardCardsLoading} badges={[{ label: "Active", value: productsPageStats?.supplierStatusBreakdown?.active ?? 0 }, { label: "Inactive", value: productsPageStats?.supplierStatusBreakdown?.inactive ?? 0 }]} />
