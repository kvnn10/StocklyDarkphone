            ...
            {loading ? (
              ...
              <div className="mt-4 grid grid-cols-1 gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
                {variants.map((variant) => {
                  ...
                  return (
                    <div key={variant.id} className="group flex min-h-[122px] flex-col rounded-xl border border-violet-300/20 bg-violet-500/[0.045] p-3 transition-colors hover:border-violet-300/35 hover:bg-violet-500/[0.07]">
                      ...
                      <div className="mt-auto flex items-end justify-between gap-2 pt-3">
                        ...
                        {stockRows.length > 0 ? (
                          <div className="flex shrink-0 flex-wrap justify-end gap-1">
                            {stockRows.map((stock) => (
                              <span key={stock.id} className="inline-flex items-center gap-1 rounded-full border border-teal-300/20 bg-teal-500/10 px-2 py-0.5 text-[10px] text-teal-700 dark:text-teal-200" title={stock.warehouseName}>
                                <MapPin className="h-3 w-3" />{stock.warehouseName}: {Math.max(0, stock.quantity - stock.reservedQuantity)}
                              </span>
                            ))}
                          </div>
