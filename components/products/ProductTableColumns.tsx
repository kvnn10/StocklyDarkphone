...
import { CopyableText, AvatarInlineLink, PersonNameEmailCell, TABLE_CATALOG_LINK_CLASS } from "@/components/shared";
...
function ProductWarehouseCell({ productId, variantsByProductId }: { productId: string; variantsByProductId?: VariantMap }) {
 ...
 for (const allocation of allocations) { const warehouseId = allocation.warehouseId; merged.set(warehouseId, { id: allocation.id, warehouseId, warehouseName: allocation.warehouse?.name ?? "Bodega", quantity: Number(allocation.quantity ?? 0), reservedQuantity: Number(allocation.reservedQuantity ?? 0) }); }
 for (const stock of variantStocks) { const existing = merged.get(stock.warehouseId); if (existing) { existing.quantity += stock.quantity; existing.reservedQuantity += stock.reservedQuantity; } else merged.set(stock.warehouseId, stock); }
...
function ProductMarginCell({ product }: { product: Product }) { const purchase = Number(product.purchasePrice ?? 0); const sale = Number(product.price ?? 0); if (sale <= 0 || purchase <= 0) return <span className="text-xs text-muted-foreground">—</span>; const profit = sale - purchase; const margin = (profit / sale) * 100; return <div className="flex min-w-[105px] flex-col gap-0.5"><span className={cn("text-xs font-semibold", profit >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400")}>{margin.toFixed(1)}%</span><span className="text-[11px] text-muted-foreground">Utilidad: ${profit.toFixed(2)}</span></div>; }
...
export function createProductColumns(detailBase: string = "", options?: CreateProductColumnsOptions): ColumnDef<Product>[] {
 const forSupplier = options?.forSupplier === true; const variantsByProductId = options?.variantsByProductId;
 return [
  { id:"product", accessorKey:"name", header:({column})=><SortableHeader column={column} label="Producto y SKU"/>, cell:({row})=>{const p=row.original; const variants=variantsByProductId?.[p.id] ?? []; return <div className="flex items-start gap-3 min-w-0 max-w-[290px]">{p.imageUrl?<SafeImage src={p.imageUrl} alt={p.name} width={48} height={48} className="h-12 w-12 shrink-0 object-cover rounded-lg border border-rose-400/30" unoptimized={p.imageUrl.includes("ik.imagekit.io")}/>:<div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-700 border"><Package className="h-5 w-5 text-muted-foreground"/></div>}<div className="flex min-w-0 flex-col gap-0.5"><Link href={detailHref(detailBase,"products",p.id)} prefetch className={cn("truncate",TABLE_CATALOG_LINK_CLASS)} title={p.name}><CopyableText value={p.name}>{p.name}</CopyableText></Link><CopyableText value={p.sku} className="truncate text-muted-foreground">{p.sku}</CopyableText>{variants.length > 0 && <div className="mt-0.5 flex min-w-0 flex-col gap-1">{variants.map((variant)=>{const available=Math.max(0,variant.quantity-variant.reservedQuantity); const color=variantColorDot(variant.name); return <div key={variant.id} className="flex min-w-0 items-center gap-1.5 text-[11px] text-violet-100" title={`${variant.name} · ${available} disponibles · SKU ${variant.sku}`}>{color ? <span aria-label={color.label} className={cn("h-2.5 w-2.5 shrink-0 rounded-full", color.className)} /> : <span className="h-2.5 w-2.5 shrink-0 rounded-full border border-violet-300/50 bg-violet-400/20" /> }<span className="truncate">{variant.name}</span><span className="font-semibold text-white/90">{available}</span></div>;})}</div>}</div></div>; }},
  { accessorKey:"quantity", header:({column})=><SortableHeader column={column} label="Stock"/>, cell:({row})=>{const p=row.original; const total=Number(p.quantity)||0; const reserved=getDisplayCommittedQuantity(p); const available=Math.max(0,total-reserved); const sold=Math.max(0,Number(p.statistics?.totalQuantitySold)||0); return <div className="flex min-w-[155px] flex-col gap-1"><div className="flex items-center gap-2"><QRCodeHover data={JSON.stringify({id:p.id,name:p.name,sku:p.sku,price:p.price,purchasePrice:p.purchasePrice,quantity:p.quantity,status:p.status,category:p.category,supplier:p.supplier})} qrCodeUrl={p.qrCodeUrl} title={p.name} size={200} iconOnly/><span className={`text-xs font-semibold ${productStockAvailableTextClass(available)}`}>{available} disponibles</span></div><div className="text-[11px] text-muted-foreground">Total: {total}</div><div className="text-[11px] text-amber-600 dark:text-amber-400">Reservado: {reserved}</div><div className="text-[11px] text-muted-foreground">Vendido: {sold}</div></div>; }},
