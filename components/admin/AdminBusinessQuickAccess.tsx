      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {cards.map((card) => {
          ...
          return <Link key={card.href} href={card.href} className={`group rounded-2xl border bg-gradient-to-br ${card.tone} p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg`}>
            <div className="flex items-start justify-between gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-background/80 shadow-sm"><Icon className="h-5 w-5" /></div><ArrowRight className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-1" /></div>
