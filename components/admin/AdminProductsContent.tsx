...
function parseText(text: string, warehouse: string, category: string): ImportRow[] {
  ...
  for (const raw of text.split(/\n+/).map((x) => x.trim()).filter(Boolean)) {
    ...
    if (variants.length >= 2) {
      const first = variants[0], prefix = cleaned.slice(0, first.index ?? 0).replace(/[,;:]\s*$/, "").replace(/^(?:tengo|compr[eé]|llegaron|agreg[aá]r?)\s+/i, "").trim();
...
export default function AdminProductsContent({ initialProducts, initialStats, initialSupplierPortal }: AdminProductsContentProps = {}) {
  ...
  const confirmImport = async () => {
    ...
    if (rows.some((r) => !r.product || !r.quantity || !r.purchasePrice || !r.price)) { setError("Completa nombre, cantidad, costo y venta antes de confirmar."); return; }
