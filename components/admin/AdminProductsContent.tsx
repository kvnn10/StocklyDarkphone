...
const colors = "negra|negro|azul|azules|blanca|blanco|natural|dorada|dorado|verde|roja|rojo|morada|morado|rosa|rosado|rosada|gris|plateada|plateado";
const modelPattern = /^(?:iPhone\s*)?(\d{1,2}(?:\s+(?:pro\s+max|pro|plus|max))?)\s*(.*?)\s*$/i;
...
function parseText(text: string, warehouse: string, category: string): ImportRow[] {
  ...
  for (const raw of lines) {
    if (/^(lista|categor[ií]a|bodega)\b/i.test(raw) || /\bde\s+cada\s+uno\b/i.test(raw)) continue;
    ...
    const sale = saleMatch?.[1] ? parseMoney(saleMatch[1] + (saleMatch[2] || "")) : 0;
    const cost = costMatch?.[1] ? parseMoney(costMatch[1] + (costMatch[2] || "")) : 0;
    const cleaned = raw
    ...
    // When the user says "1 de cada uno", the numbers at the beginning are model numbers,
    // not quantities. Example: "15 Pro todo sin display y máquina" => iPhone 15 Pro / detail.
    if (eachQuantity !== null) {
      const model = cleaned.match(modelPattern);
      if (!model?.[1]) continue;
...
export default function AdminProductsContent({ initialProducts, initialStats, initialSupplierPortal }: AdminProductsContentProps = {}) {
  ...
  const preview = () => {
    ...
    if (!parsed.length) { setError("No pude identificar productos. Prueba con una línea como “15 Pro todo sin display” o “20 baterías iPhone 13 a 33 mil, venta 80 mil”."); return; }
